...
public class ElasticJobConfig {
    ...
    public JobScheduler simpleJobScheduler(ZookeeperRegistryCenter regCenter, JobEventConfiguration jobEventConfiguration) {
        return new SpringJobScheduler(new JobForDay(), regCenter, getLiteJobConfiguration(), jobEventConfiguration);
    }
    ...
    private LiteJobConfiguration getLiteJobConfiguration() {
        // 定义作业核心配置
        JobCoreConfiguration simpleCoreConfig = JobCoreConfiguration.newBuilder("JobForDay", "0/3 * * * * ?", 1).build();
        // 定义SIMPLE类型配置
